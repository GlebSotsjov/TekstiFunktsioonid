<div id="yl1">
    ...
    $colors = [
        ...
        "Pink", "Gray", "Purple", "Black",
        "Aqua", "Orange", "Cyan", "Tomato", "Indigo", "IndianRed"
    ];

    for ($i = 0; $i < count($colors); $i++) {
        echo "<span style='color: $colors[$i];'><br>$colors[$i]</span>";
        echo "<div style='background-color: $colors[$i] width 75%'</div>";
    }
    ?>
</div>
...
"1N6AA0EDXFN868772", "WBADW3C59DJ422810"
    ...
    if(count($autod) == count($VIN)){
        echo "Massiivid on ühepikkulised";
    }
    else{
        echo "Massiivid ei ole ühepikkulised";
    }
    echo "<br>";
    ...
    echo "Audide arv: ".$audiArv."<br><br>";
    for ($i = 0; $i < count($VIN); $i++) {
        if (sterlen($VIN[$i]) < 17) {
            echo $autod[$i]."-".$VIN[$i]."<br>";
        }
    }
    echo "<br>";
    echo "<br>";
...
//<div id="yl3">
    ...
    //$pildiArray =array();
    //foreach ($asukoht as $
     //   array_push($pildiArray, $rida);
    //}
    //for ($m = 0; $m < count($pildiArray); $m++) {
        //echo "<img src=.PildidArray[$m]'Width='100px' alt=''>";
    //}
    ?>
</div>
